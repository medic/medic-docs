...
Applications built with the framework can have automated and prioritized lists of upcoming tasks, covering all of a health worker's activities in their community. Health workers are guided through tasks — such as screening for high-risk pregnancies or diagnosing and providing treatments for children — and get real-time indicators for progress towards their goals. These workflows are highly flexible and developed using a combination of XForms, JavaScript, and JSON. The XForms portion extends the ODK XForm notation, and is used to define forms to create contacts, as well as to define actions for these contacts, such as surveys, decision support protocols. The JavaScript and JSON portions allow for full control of tasks, targets, and contact profiles, as well as other application settings.
...
## Forms
Whether using your application in the browser or via an Android app, all Actions, Tasks, Contact creation/edit forms are defined using [ODK XForms](https://opendatakit.github.io/xforms-spec/) -- a XML definition of the structure and format for a set of questions. Since writing raw XML can be tedious, we suggest creating the forms using the [XLSForm standard](http://xlsform.org/), and using the [medic-conf](https://github.com/medic/medic-conf) command line configurer tool to convert them to XForm format. The instructions below assume some knowledge of XLSForm, and distinguish between Contact forms, those used to create and edit contacts, and App forms, those used for surveys and decision support.
...
### App Forms
Workflows within your app rely heavily on forms. When a user completes an Action or Task form the contents are saved in the database with the type `data_record`. These docs are known as _Reports_. 
